...
import { kanbanMenu, kanbanProjects, kanbanCurrentUser } from "../data/KanbanDashboardData";

const KanbanDashboardSidebar = () => {
    return (
        <aside className="w-64 bg-white dark:bg-surface-dark border-r border-gray-200 dark:border-border-dark flex-shrink-0 flex flex-col h-full overflow-y-auto">
            <div className="p-6">
                <div className="flex items-center space-x-3 mb-8">
                    ...
                    <div>
                        <h1 className="font-bold text-xl leading-tight tracking-tight">Taskio</h1>
                    ...
                    <nav className="space-y-1">
                        {kanbanMenu.map((item) => (
                            <a key={item.id} className={`flex items-center space-x-3 ${item.active ? 'text-white bg-card-dark' : 'text-text-secondary hover:text-white hover:bg-card-dark'} px-3 py-2.5 rounded-xl transition group`} href="#">
                                <span className={`material-symbols-outlined text-[22px] ${item.active ? 'text-white' : 'group-hover:text-white'}`}>{item.icon}</span>
                ...
                <div>
                    ...
                    <div className="space-y-1 mt-2">
                        <div className="pl-4 ml-2 border-l border-border-dark space-y-3 py-1">
                            {kanbanProjects.map((project) => (
                                <a key={project.id} className={`flex items-center space-x-3 ${project.active ? 'text-white' : 'text-text-secondary hover:text-white'} group pl-4 py-1 relative`} href="#">
            ...
            <div className="mt-auto p-6 pt-0">
                ...
                <div className="bg-card-dark rounded-xl p-3 flex items-center justify-between border border-border-dark hover:border-gray-600 transition cursor-pointer">
                    <div className="flex items-center space-x-3">
                        <img alt="User Avatar" className="w-9 h-9 rounded-full object-cover" src={kanbanCurrentUser.avatar} />
                        <div className="overflow-hidden">
                            <p className="text-sm font-semibold text-white truncate">{kanbanCurrentUser.name}</p>
                            <p className="text-xs text-text-secondary truncate">{kanbanCurrentUser.role}</p>
                        </div>
                    </div>
...
export default KanbanDashboardSidebar;
